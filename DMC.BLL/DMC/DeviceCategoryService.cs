...
namespace DMC.BLL
{
    ...
    {
        private DeviceCategoryDAL _dal = new DeviceCategoryDAL();
        ...
        {
            return pageView.PageView("t_DeviceCategory", "AutoId", pageIndex, pageSize, "*", "AutoId DESC", Where, out total, out pageCount);
        }
        ...
        /// <returns></returns>
        public string NewDeviceCategory(DeviceCategoryEntity entity)
        {
            ...
            try
            {
                ...
                {
                    msg = "设备类别编号或名称已存在，无需再添加";
                }
                else if (!_dal.NewDeviceCategory(entity))
                {
                    msg = "新增设备分类保存数据库失败";
                }
            }
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public string UpdateDeviceCategory(DeviceCategoryEntity entity)
        {
            string msg = string.Empty;
            try
            {
                ...
                else if (!_dal.UpdateDeviceCategory(entity))
                {
                    msg = "更新设备分类保存数据库失败";
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public StringBuilder GetDeviceTree()
        {
            StringBuilder result = new StringBuilder();
            result.Append("[{\"id\":\"\",\"text\":\"主分类\",\"attributes\":{\"categorytext\":\"\"}");
            DataTable allDeviceCategory = _dal.GetAllDeviceCategory();
            if (allDeviceCategory != null && allDeviceCategory.Rows.Count > 0)
            {
                result.AppendFormat(",\"children\":{0}", GetDeviceTreeNode("", allDeviceCategory));
            }
            result.Append("}]");
            return result;
        }
        ...
        protected StringBuilder GetDeviceTreeNode(string PCategoryId, DataTable dt)
        {
            ...
            foreach (DataRow dr in drList)
            {
                ...
                //子分类设置
                StringBuilder childrenStr = GetDeviceTreeNode(dr["CategoryId"].ToString(), dt);
