<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DeviceCategory.aspx.cs" Inherits="Web.UI.Equment.DeviceCategory" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#viewtree").tree({
                lines: true,
                onSelect: function (node) {
                    if (veditfatherid != null) {
                        var node = $('#viewtree').tree('find', veditfatherid);
            ...
            //綁定datagrid
            $('#tbDeviceCategory').datagrid({
                //是否折?
                collapsible: true,
                checkOnSelect: false,
                selectOnCheck: false,
                url: '../../ASHX/DMC/DeviceCategory.ashx?M=Search',
                //數據在一行顯示 
                nowrap: false,
                fitColumns: true,
                //行條紋化
                ...
                //可動列
                columns: [[
                           { field: 'categoryid', title: '类别编号', width: 50, align: 'left' },
                           { field: 'categoryname', title: '类别名称', width: 50, align: 'left' },
                           { field: 'orderid', title: '排序序号', width: 50, align: 'left' },
                           { field: 'pcategoryid', title: '父级类别', width: 50, align: 'left' },
            ...
            $(window).resize(function () {
                $('#tbDeviceCategory').datagrid('resize');
            });
            //設置分页控件屬性 
            var p = $('#tbDeviceCategory').datagrid('getPager');
            $(p).pagination({
                pageSize: 10,
        ...
        function Search(SearchType) {
            var queryParams = $('#tbDeviceCategory').datagrid('options').queryParams;
            //請輸入關鍵字            
            ...
            $('#tbDeviceCategory').datagrid('reload');
        }
        //單擊高級查詢按鈕
        ...
        var url = "../../ASHX/DMC/DeviceCategory.ashx?M=NewDeviceCategory";
        //归属设置
        function Edit(index) {
            url = "../../ASHX/DMC/DeviceCategory.ashx?M=UpdateDeviceCategory";
            $('#fm').form('clear');
            var row = $('#tbDeviceCategory').datagrid('getData').rows[index];
            $('#fm').form('load', row);
            PCategoryId = null;
            PCategoryText = null;
            editfatherid = row.pcategoryid;
            LoadTree();
            $('#divNew').dialog('open').dialog('setTitle', '修改类别');
        }
        //设备详情
        function View(index) {
            $('#fmView').form('clear');
            ...
            veditfatherid = row.pcategoryid;
            $('#divView').dialog('open').dialog('setTitle', '类别详情');
            LoadViewTree();
        }
        ...
        function LoadTree() {
            $('#tree').tree('options').url = "../../ASHX/DMC/DeviceCategory.ashx?M=GetDeviceTree";
            $('#tree').tree('reload');
        }

        function LoadViewTree() {
            $('#viewtree').tree('options').url = "../../ASHX/DMC/DeviceCategory.ashx?M=GetDeviceTree";
            $('#viewtree').tree('reload');
        }
        function add() {
            url = "../../ASHX/DMC/DeviceCategory.ashx?M=NewDeviceCategory";
            $('#fm').form('clear');
            PCategoryId = null;
            PCategoryText = null;
            editfatherid = null;
            ...
            $('#divNew').dialog('open').dialog('setTitle', '新增类别');
        }
    </script>
</head>
<body>
    <div id="funMain" style="background-color: transparent; margin-left: 5px; margin-right: 0px;">
        ...
        <div class="squery" id="divSearch">
            <div class="sinquery">
                <table class="addCoyTB" style="height: 30px; border: 0px;">
                    <tr>
                        <td style="width: 70px;">类别名称:</td>
                        <td style="width: 195px;">
                            <input class="easyui-textbox" id="qCategoryName" data-options="prompt:'请输入类别名称'" style="width: 150px;" />
                        </td>
        ...
        <table id="tbDeviceCategory" data-options="fit:false">
        </table>
    </div>
    ...
    <div id="divNew" class="easyui-dialog" data-options="closed:true,modal:true,maximizable:false,buttons:'#dlg-button'"
        ...
        <div class="easyui-layout" data-options="fit:true,border:false">
            <div data-options="region:'west',split:true,title:'父级类别',collapsible:false" style="width: 30%;">
                <ul id="tree" class="easyui-tree" fit="true" data-options="animate:true,dnd:false">
                </ul>
            </div>
            <div data-options="region:'center',split:true,title:'类别明细维护',collapsible:false" style="width: 70%;">
                <form id="fm">
                    <table cellpadding="0" cellspacing="0" class="tbStyle" style="width: 100%; height: 90%;">
                        <tr>
                            <td>类别编号：
                            </td>
                            <td>
    ...
    <div id="dlg-button">
        <a class="easyui-linkbutton" href="javascript:void(0)" onclick="Save()">保存</a>
        <a class="easyui-linkbutton" href="javascript:void(0)" onclick="closeWindow('divNew');">关闭</a>
    </div>

    <div id="divView" class="easyui-dialog" data-options="closed:true,modal:true,maximizable:false,buttons:'#dlg-buttonview'"
        style="width: 620px; height: 310px; padding: 3px;">
        <div class="easyui-layout" data-options="fit:true,border:false">
            <div data-options="region:'west',split:true,title:'父级类别',collapsible:false" style="width: 30%;">
                <ul readonly="readonly" id="viewtree" class="easyui-tree" data-options="animate:true,dnd:false,fit:true">
                </ul>
            </div>
            <div data-options="region:'center',split:true,title:'类别明细维护',collapsible:false" style="width: 70%;">
                <form id="fmView">
                    <table cellpadding="0" cellspacing="0" class="tbStyle" style="width: 100%; height: 90%;">
                        <tr>
                            ...
                            <td>
                                <input disabled="disabled" type="text" name="categoryid" class="easyui-textbox" data-option="required:true" /><span style="color: red;">*</span>
                            </td>
                        </tr>
                        <tr>
                            <td>类别名称：
                            ...
                            <td>
                                <input disabled="disabled" type="text" name="categoryname" class="easyui-textbox" data-option="required:true" /><span style="color: red;">*</span>
                            </td>
                        </tr>
                        <tr>
                            <td>排序序号：
                            ...
                            <td>
                                <input disabled="disabled" type="text" name="orderid" class="easyui-numberbox" data-option="required:true" id="Text3" />
                            </td>
                        </tr>
                        <tr>
                            <td>有效否：
                            ...
                            <td>
                                <select disabled="disabled" class="easyui-combobox" style="width: 100px; font-size: 12px;" name="usey">
                                    <option value="Y" selected="selected">Y</option>
                                    <option value="N">N</option>
                                </select>
                            </td>
    ...
    <div id="dlg-buttonview">
        <a class="easyui-linkbutton" href="javascript:void(0)" onclick="closeWindow('divView');">关闭</a>
    </div>
</body>
</html>
